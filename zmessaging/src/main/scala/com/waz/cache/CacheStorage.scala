...
class CacheStorage(storage: Database, context: Context) extends CachedStorage[String, CacheEntryData](new EntryCache(context), storage)(CacheEntryDao, "CacheStorage") {
  ...
  override def get(key: String): Future[Option[CacheEntryData]] = {
    super.get(key) map {
...
object CacheStorage {
  ...
  class EntryCache(context: Context) extends TrimmingLruCache[String, Option[CacheEntryData]](context, Fixed(1024 * 1024) min Relative(.05f)) {
    override def sizeOf(key: String, value: Option[CacheEntryData]): Int = value.flatMap(_.data).fold(0)(_.length) + key.length + value.flatMap(_.path).fold(0)(_.getPath.length) + 56 // data plus some object overhead
  }
}
