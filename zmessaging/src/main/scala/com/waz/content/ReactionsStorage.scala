...
class ReactionsStorage(context: Context, storage: Database) extends CachedStorageImpl[(MessageId, UserId), Liking](new TrimmingLruCache(context, Fixed(DefaultMessagesStorage.cacheSize)), storage)(LikingDao, "LikingStorage") {
