...
class DownloadQueue {
  ...
  val entries = new mutable.HashMap[DownloadRequest, Entry]
  val queue = new mutable.PriorityQueue[Entry]()(EntryOrdering)

  def put(key: DownloadRequest, uiWaiting: Boolean = true, timestamp: Long = System.currentTimeMillis()) = {
  ...
  def peek(): Option[DownloadRequest] = {
    dropDeleted()
...
object DownloadQueue {
  ...
  case class Entry(key: DownloadRequest, uiWaiting: Boolean, timestamp: Long) {
    private[downloads] var deleted = false
  }
