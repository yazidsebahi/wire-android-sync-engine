...
class ImageAssetGenerator(context: Context, cache: CacheService, loader: ImageLoader, imageCache: MemoryImageCache, bitmapLoader: BitmapDecoder) {
  ...
  /**
    ...
    **/
  def generateWireAsset(local: AssetData, profilePicture: Boolean): CancellableFuture[AssetData] = {
    verbose(s"generateWireAsset: $local, profilePicture?: $profilePicture")
    loader.loadRawImageData(local) flatMap {
      case Some(data) =>
        loader.getImageMetadata(data) flatMap { meta => generateAssetData(local.id, Left(data), meta, if (profilePicture) SelfOptions else ImageOptions) }
      case _ =>
        CancellableFuture.failed(new IllegalArgumentException(s"ImageAsset could not be added to cache: $local"))
    }
  }
  ...
  def generateAssetData(assetId: AssetId, input: Either[LocalData, Bitmap], meta: Metadata, co: CompressionOptions): CancellableFuture[AssetData] = {
    generateImageData(assetId, co, input, meta) flatMap {
      case (file, m) =>
