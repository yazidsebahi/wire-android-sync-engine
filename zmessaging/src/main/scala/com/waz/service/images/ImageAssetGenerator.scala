...
class ImageAssetGenerator(context: Context, cache: CacheService, loader: ImageLoader, imageCache: MemoryImageCache, bitmapLoader: BitmapDecoder) {
  ...
  /**
    ...
    **/
  def generateWireAsset(local: AssetData, convId: Option[RConvId], profilePicture: Boolean): CancellableFuture[AssetData] =
  loader.loadRawImageData(local, convId) flatMap {
    case Some(data) =>
