...
class AsyncClientWebSpec extends FeatureSpecLike with Matchers with BeforeAndAfter with RobolectricTests with ScalaFutures with DefaultPatienceConfig {
  ...
  feature("Get") {
    ...
    scenario("GET https from https://wire.com") {
      Await.result(client(utils.URI.parse("https://www.wire.com"), timeout = AsyncClient.DefaultTimeout), 5.second) match {
    ...
    scenario("Get gzipped content from www.gradle.com") {
      Await.result(client(utils.URI.parse("http://www.gradle.com"), timeout = AsyncClient.DefaultTimeout), 5.second) match {
        case Response(HttpStatus(200, _), _, _) => //expected
        case res => fail(s"got unexpected response: $res")
      }
    }
    scenario("Get gzipped chunked content from www.clockworkmod.com") {
      Await.result(client(utils.URI.parse("http://www.clockworkmod.com"), timeout = AsyncClient.DefaultTimeout), 5.second) match {
        case Response(HttpStatus(200, _), _, _) => //expected
        case res => fail(s"got unexpected response: $res")
      }
    }
  ...
  feature("Post") {
    ...
    scenario("post data") {
      ...
      Await.result(client(utils.URI.parse("http://posttestserver.com/post.php"), "POST", new BinaryRequestContent(data, "text/plain"), timeout = AsyncClient.DefaultTimeout), 45.seconds) match {
        case Response(HttpStatus(200, _), _, _) => //expected
        case res => fail(s"got unexpected response: $res")
      }
    }
    ...
    scenario("post file with len") {
      ...
      Await.result(client(utils.URI.parse("http://posttestserver.com/post.php"), "POST", new StreamRequestContent(new FileInputStream(file), "image/png", file.length.toInt), timeout = AsyncClient.DefaultTimeout), 45.seconds) match {
        case Response(HttpStatus(200, _), _, _) => //expected
        case res => fail(s"got unexpected response: $res")
      }
    }
