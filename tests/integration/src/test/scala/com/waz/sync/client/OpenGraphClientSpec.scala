...
class OpenGraphClientSpec extends FeatureSpec with Matchers with ProvisionedApiSpec with ScalaFutures with DefaultPatienceConfig {
  ...
  scenario("Fetch header of large website, should stop download early") {
    lazy val LargeWebsiteLink = utils.URI.parse("http://unicode.org/emoji/charts/full-emoji-list.html")
  ...
  scenario("Load open graph data from wire.com") {
    val resp = client.loadMetadata(utils.URI.parse("http://www.wire.com")).await()
    resp shouldBe 'right
  ...
  scenario("Load open graph data from twitter") {
    val resp = client.loadMetadata(utils.URI.parse("https://twitter.com/SoundCloud/status/750403925585432576")).await()
    resp shouldBe 'right
    resp.right.get shouldBe defined
    val data = resp.right.get.get
    info(s"loaded data: $data")
  ...
  scenario("Load open graph data from nytimes") {
    val resp = client.loadMetadata(utils.URI.parse("http://nytimes.com/2016/07/11/business/front-page-editorials-aim-to-soothe-the-grief-stricken.html")).await()
    resp shouldBe 'right
    resp.right.get shouldBe defined
    val data = resp.right.get.get
    info(s"loaded data: $data")
