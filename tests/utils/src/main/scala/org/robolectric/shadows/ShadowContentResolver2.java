...
public class ShadowContentResolver2 {
    ...
    private Map<Uri, Function0<InputStream>> inputStreamMap = new HashMap<Uri, Function0<InputStream>>();
    ...
    public void registerInputStream(Uri uri, Function0<InputStream> inputStream) {
        inputStreamMap.put(uri, inputStream);
    }

    @Implementation
    public final InputStream openInputStream(final Uri uri) {
        Function0<InputStream> inputStream = inputStreamMap.get(uri);
        if (inputStream != null) {
...
//    }
    ...
    public static boolean isSyncActive(Account account, String authority) {
        ShadowContentResolver2.Status status = getStatus(account, authority);
    ...
    private static class UnregisteredInputStream extends InputStream implements NamedStream {
        ...
        public int read() throws IOException {
            throw new UnsupportedOperationException("You must use ShadowContentResolver2.registerInputStream() in order to call read()");
        }
