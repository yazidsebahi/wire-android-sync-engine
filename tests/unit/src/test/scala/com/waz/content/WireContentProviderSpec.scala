...
class WireContentProviderSpec extends FeatureSpec with Matchers with BeforeAndAfter with RobolectricTests with RobolectricUtils with ScalaFutures with DefaultPatienceConfig { test =>
  ...
  feature("Cache uri") {
    ...
    scenario("Query cache entry metadata") {
      ...
      val c = provider.query(utils.URI.unwrap(uri), Array(OpenableColumns.DISPLAY_NAME, OpenableColumns.SIZE), null, null, null, null)
    ...
    scenario("Query without specified project") {
      val c = provider.query(utils.URI.unwrap(uri), null, null, null, null, null)
      c.getCount shouldEqual 1
      c.moveToFirst()
    ...
    scenario("Load file data") {
      IoUtils.toByteArray(new FileInputStream(provider.openFile(utils.URI.unwrap(uri), "r").getFileDescriptor)).toSeq shouldEqual assetData.toSeq
    }
