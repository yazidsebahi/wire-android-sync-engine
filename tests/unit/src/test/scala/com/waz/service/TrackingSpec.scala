...
class TrackingSpec extends FeatureSpec with Matchers with BeforeAndAfter with BeforeAndAfterAll with RobolectricTests with RobolectricUtils { test =>
  ...
  lazy val otto = UserData(UserId(), "Otto", Some(EmailAddress("welcome+36@wire.com")), None, None, None, 0, SearchKey("Otto"), ConnectionStatus.Accepted, new Date, None, Some(ottoConv.remoteId))
  ...
  override protected def beforeAll(): Unit = {
    ...
    sequence(
      Vector(
        ...
        zmessaging.messagesStorage.insert(MessageData(MessageId(), ottoConv.id, EventId(0), EventId(0), Message.Type.MEMBER_JOIN, selfUser.id)),
        zmessaging.messagesStorage.insert(MessageData(MessageId(), ottoConv.id, EventId(1), EventId(1), Message.Type.MEMBER_JOIN, otto.id)),
        zmessaging.messagesStorage.insert(MessageData(MessageId(), ottoConv.id, EventId(2), EventId(2), Message.Type.TEXT, otto.id)),
        zmessaging.messagesStorage.insert(MessageData(MessageId(), oneToOne.id, EventId(0), EventId(30), Message.Type.ASSET, selfUser.id)),
  ...
  feature("Update tracking data") {
    ...
    scenario("Send text to bot") {
      zmessaging.messagesStorage.insert(MessageData(MessageId(), ottoConv.id, EventId(3), EventId(3), Message.Type.TEXT, selfUser.id))
      tracking should resemble(TrackingStats(4, 2, 1, 1, 1, 2, 42, 0, 1, 1, 1)) soon
    }

    scenario("Send image somewhere") {
      zmessaging.messagesStorage.insert(MessageData(MessageId(), oneToOne.id, EventId(1), EventId(1), Message.Type.ASSET, selfUser.id))
