...
class PushServiceSpec extends AndroidFreeSpec { test =>
  ...
  scenario("Ensure that push service notification pipeline executes atomically") {
    ...
    val notsHandler = Signal[() => Future[Unit]]
    (notificationStorage.registerEventHandler (_: () => Future[Unit])(_: EventContext)).expects(*, *).onCall { (handler: (() => Future[Unit]), ec: EventContext) =>
    ...
    val pipeline = new EventPipeline {
      override def apply(input: Traversable[Event]) = {
        Future.successful(verbose("pipeline executing"))
      }
    }
