...
class ArchivingAndMutingSpec extends FeatureSpec with Matchers with BeforeAndAfter with OptionValues with RobolectricTests with RobolectricUtils { test =>
  ...
  def sendImageMessage[T](old: Boolean = false): Unit =
    service.dispatchEvent(GenericAssetEvent(Uid(), conv.remoteId, date(old), selfUser.id, GenericMessage(Uid(), ImageAsset("preview", 0, 0, 0, 0, "", 1, None, None)), RAssetId(), None))
