...
class NotificationServiceSpec extends FeatureSpec with Matchers with PropertyChecks with BeforeAndAfter with BeforeAndAfterAll with RobolectricTests with RobolectricUtils with DefaultPatienceConfig { test =>
  ...
  feature("Add notifications for events") {
    ...
    scenario("Process user connection event for an unsynced user") {
      ...
      withDelay {
        currentNotifications should beMatching {
          case Seq(NotificationInfo(NotificationType.CONNECT_REQUEST, "hello", _, Some("other user"), Some("other user"), false, _, _)) => true
        }
      }
    }
    ...
    scenario("Process contact join event") {
      ...
      withDelay {
        currentNotifications should beMatching {
          case Seq(NotificationInfo(NotificationType.CONTACT_JOIN, _, _, _, _, false, _, _)) => true
        }
      }
    }
    ...
    scenario("Process mentions event") {
      ...
      withDelay {
        currentNotifications should beMatching {
          case Seq(NotificationInfo(NotificationType.TEXT, "test name", `convId`, _, _, false, true, _)) => true
        }
      }
    }
    ...
    scenario("Process any asset event") {
      ...
      withDelay {
        currentNotifications should beMatching {
          case Seq(NotificationInfo(NotificationType.ANY_ASSET, _, `convId`, _, _, false, _, _)) => true
        }
      }
    }
    ...
    scenario("Receiving the same message twice") {
      ...
      withDelay {
        currentNotifications should beMatching {
          case Seq(NotificationInfo(NotificationType.TEXT, "meep", `groupId`, Some("group conv"), _, true, _, _)) => true
        }
      }
    }
  ...
  feature("List notifications") {
    ...
    scenario("List notifications with local conversation ids") {
      ...
      withDelay {
        currentNotifications should beMatching {
          case Seq(NotificationInfo(NotificationType.TEXT, _, `groupId`, Some("group conv"), _, true, _, _)) => true
        }
      }
    }
  }
