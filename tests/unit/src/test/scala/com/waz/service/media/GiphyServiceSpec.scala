...
class GiphyServiceSpec extends FeatureSpec with Matchers with BeforeAndAfter with RobolectricTests with RobolectricUtils { test =>
  ...
  val biggestRandomResult = AssetData(metaData = Some(Image(Dim2(500, 256), Medium)), mime = Mime.Image.Gif, source =  Some(utils.URI.parse("http://s3.amazonaws.com/giphygifs/media/Ggjwvmqktuvf2/giphy.gif")))
  val smallestRandomResult = AssetData(metaData = Some(Image(Dim2(100, 51), Preview)), mime = Mime.Image.Gif, source =  Some(utils.URI.parse("http://s3.amazonaws.com/giphygifs/media/Ggjwvmqktuvf2/100w_s.gif")))
  val randomResult = (Some(smallestRandomResult), biggestRandomResult)

  var loadSearchResult: Seq[(Option[AssetData], AssetData)] = Nil
  var keyword: String = ""
  val biggestSearchResult = AssetData(sizeInBytes = ImageAssetGenerator.MaxGifSize - 1, metaData = Some(Image(Dim2(391, 200), Medium)), mime = Mime.Image.Gif, source =  Some(utils.URI.parse("http://s3.amazonaws.com/giphygifs/media/Ggjwvmqktuvf2/200_d.gif")))
  val smallestSearchResult = AssetData(metaData = Some(Image(Dim2(100, 120), Preview)), mime = Mime.Image.Gif, source =  Some(utils.URI.parse("http://s3.amazonaws.com/giphygifs/media/Ggjwvmqktuvf2/200_d.gif")))
