...
class ConversationsSyncHandlerSpec extends FeatureSpec with Matchers with ScalaFutures with ScaledTimeSpans with BeforeAndAfter with GivenWhenThen with RobolectricTests with RobolectricUtils with DefaultPatienceConfig { test =>
    ...
    service = new MockZMessaging() {
      override lazy val conversations: ConversationsService =
        new ConversationsService(context, push, users, usersStorage, messagesStorage, membersStorage,
          convsStorage, convsContent, convsStats, membersContent, sync, errors, messages, imageAssets, storage, messagesContent, keyValue, eventScheduler) {
  ...
  feature("Error reporting") {
    ...
    scenario("add unconnected user to conversation") {
      ...
      Await.result(handler.postConversationMemberJoin(conv.id, Seq(user1, user2)), 1.second) shouldEqual SyncResult.Failure(Some(postMemberJoinResponse.left.get), shouldRetry = false)
      service.convsStorage.get(conv.id) should eventually(be('defined))
