...
class SpotifyClientSpec extends FeatureSpec with Matchers with JsonResponseFromResources with OptionValues with RobolectricTests {
  ...
  feature("response parsing") {
    scenario("parse track response") {
      ...
      val images = TrackResponse.unapply(sampleTrackResponse).value.images.headOption.value.versions

      images.map(img => (img.tag, img.url.value, img.width, img.height)) should contain theSameElementsInOrderAs Seq(
    ...
    scenario("parse playlist response") {
      ...
      val images = assets.find(a => pl.artwork.contains(a.id)).value.versions
      images should have size 1
      images.headOption.map(i => (i.width, i.height, i.tag, i.url.value)).value shouldEqual (0, 0, "0", "https://u.scdn.co/images/pl/default/02c1007d606188e383f00571520d97e488c7dc28")
    }
  }
